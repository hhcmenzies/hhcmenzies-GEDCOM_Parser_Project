\
    ...
    This script is intentionally lightweight and safe to run before every scrape.
    It can also (optionally) make *live* Wikipedia API calls to verify that each
    configured source title resolves and that categories aren't empty.

    Usage:
      python preflight_ma_notables.py --config collections.json --show

      # Live checks (requires network + a descriptive UA)
    ...
    Notes:
    - Wikipedia API will return HTTP 403 if you do not send a proper User-Agent.
    - This preflight DOES NOT scrape people; it only validates configuration inputs.
    """
    ...
    def norm_title(t: str) -> str:
        # Strip whitespace; keep punctuation (Wikipedia uses it for exact titles).
    ...
    class WikiClient:
        ...
        def _get(self, params: Dict[str, Any]) -> requests.Response:
            ...
            r = self.session.get("https://en.wikipedia.org/w/api.php", params=params, timeout=30)
            return r
        ...
        def resolve_title(self, title: str) -> ResolveResult:
            ...
            if page.get("missing"):
                return ResolveResult(False, r.status_code, None)
            # normalized title may be in redirects / normalized section; easiest is page["title"]
        ...
        def category_member_count(self, category_title: str, limit_probe: int = 1) -> int:
            """
            Counts page members (namespace 0) in a category. This is a *probe*:
            we only need to know if it's empty/non-empty, not full counts.
            """
            ct = category_title
            ...
            r = self._get({
                ...
                "cmtitle": ct,
                "cmlimit": str(max(1, min(500, limit_probe))),
                "cmtype": "page",
                "formatversion": 2,
            })
            r.raise_for_status()
    ...
    def main() -> int:
        ...
        ap.add_argument("--config", default="collections.json", help="Path to collections.json")
        ap.add_argument("--show", action="store_true", help="Print config summary and exit (no network)")
        ap.add_argument("--user-agent", dest="user_agent", default="", help="Wikipedia User-Agent (required for --check-wikipedia)")
        ap.add_argument("--check-wikipedia", action="store_true", help="Resolve configured titles via Wikipedia API")
        ap.add_argument("--sleep", type=float, default=0.0, help="Sleep seconds between API calls (politeness)")
        ap.add_argument("--max-titles", type=int, default=500, help="Max number of source titles to check")
        args = ap.parse_args()
        ...
        for c in cols:
            ...
            # soft warning if outside project window
        ...
        # Shays sanity: avoid the recurring typo
        for c in cols:
            ...
            for s in sh_sources:
                if not isinstance(s, dict):
                    continue
                t = s.get("title") or ""
                effective.append({"type": s.get("type"), "title": t, "traverse_subcategories": bool(s.get("traverse_subcategories", False))})
            info(f"EFFECTIVE SHAYS SOURCES: {effective}")
            # enforce list_page title
            for s in sh_sources:
        ...
        if args.check_wikipedia:
            ...
            checked = 0
            missing_titles: List[Tuple[str, str, str, int]] = []  # (slug, source_type, title, http)
            ...
            for c in cols:
                ...
                for s in c.get("sources", []) or []:
                    ...
                    checked += 1
                    if not res.ok:
                        missing_titles.append((slug, st or "unknown", t, res.http_status))
                        warn(f"{slug}: {st} '{t}' -> NOT OK (HTTP {res.http_status})")
                        continue

                    if res.normalized_title and res.normalized_title != t:
                        normalized_map.append((slug, st or "unknown", t, res.normalized_title))
            ...
            if normalized_map:
                info("Titles normalized by Wikipedia (showing up to 50):")
