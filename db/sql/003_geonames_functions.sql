...
AS $$
  ...
  CROSS JOIN q
  WHERE
    -- **CRITICAL**: if query says USA/US, restrict search to US
    (q.hint3 NOT IN ('USA','US') OR a.country_code='US')
    AND (a.name % q.token OR a.asciiname % q.token)
    -- optional: avoid junk features early (keeps performance sane)
